...
def _silver_customers(raw: list[dict]) -> list[dict]:
    seen: set[str] = set()
    result: list[dict] = []
    for r in raw:
        if int(r["age"]) > 0 and int(r["tenure_days"]) >= 0 and r["customer_id"] not in seen:
            seen.add(r["customer_id"])
            result.append(dict(r))
    return result


def _aggregate_revenue(orders: list[dict]) -> list[dict]:
...
def _aggregate_products(orders: list[dict]) -> list[dict]:
    perf: dict[str, dict] = {}
    for order in orders:
        if order["status"] != "completed":
            continue
    ...
    return sorted(perf.values(), key=lambda r: r["gross_revenue"], reverse=True)
...
def demo_data() -> tuple[list[dict], list[dict], list[dict]]:
    ...
    # Silver: filter + deduplicate
    silver_customers = _silver_customers(raw_customers)
    silver_orders = [
        dict(r)
        for r in raw_orders
    ...
    # Gold: aggregations
    gold_revenue = _aggregate_revenue(silver_orders)
    gold_products = _aggregate_products(silver_orders)
